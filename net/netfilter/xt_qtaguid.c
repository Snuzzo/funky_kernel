...
static struct tag_node *tag_node_tree_search(struct rb_root *root, tag_t tag)
{
	...
	while (node) {
		...
		int result;
		RB_DEBUG("[NET] qtaguid: tag_node_tree_search(0x%llx): "
			 " node=%p data=%p\n", tag, node, data);
...
static void tag_node_tree_insert(struct tag_node *data, struct rb_root *root)
{
	...
	while (*new) {
		...
		RB_DEBUG("[NET] qtaguid: %s(): tag=0x%llx"
			 " (uid=%u)\n", __func__,
			 this->tag,
...
struct uid_tag_data *get_uid_data(uid_t uid, bool *found_res)
{
	...
	DR_DEBUG("[NET] qtaguid: get_uid_data(%u) utd=%p\n", uid, utd_entry);
	...
	if (!utd_entry) {
		pr_err("[NET] qtaguid: get_uid_data(%u): "
	...
	DR_DEBUG("[NET] qtaguid: get_uid_data(%u) new utd=%p\n", uid, utd_entry);
	return utd_entry;
}
				   ...
				   struct uid_tag_data *utd_entry)
{
	...
	if (utd_entry->num_active_tags + 1 > max_sock_tags) {
		pr_info("[NET] qtaguid: new_tag_ref(0x%llx): "
	...
	if (!tr_entry) {
		pr_err("[NET] qtaguid: new_tag_ref(0x%llx): "
	...
	DR_DEBUG("[NET] qtaguid: new_tag_ref(0x%llx): "
				      ...
				      struct uid_tag_data **utd_res)
{
	...
	DR_DEBUG("[NET] qtaguid: lookup_tag_ref(tag=0x%llx (uid=%u))\n",
		 full_tag, uid);
	...
	if (utd_res)
		*utd_res = utd_entry;
	DR_DEBUG("[NET] qtaguid: lookup_tag_ref(0x%llx) utd_entry=%p tr_entry=%p\n",
				   ...
				   struct uid_tag_data **utd_res)
{
	...
	DR_DEBUG("[NET] qtaguid: get_tag_ref(0x%llx)\n",
		 full_tag);
	...
	if (utd_res)
		*utd_res = utd_entry;
	DR_DEBUG("[NET] qtaguid: get_tag_ref(0x%llx) utd=%p tr=%p\n",
		 full_tag, utd_entry, tr_entry);
	return tr_entry;
}
...
static void free_tag_ref_from_utd_entry(struct tag_ref *tr_entry,
					struct uid_tag_data *utd_entry)
{
	DR_DEBUG("[NET] qtaguid: %s(): %p tag=0x%llx (uid=%u)\n", __func__,
	...
	if (!tr_entry->num_sock_tags) {
		...
		DR_DEBUG("[NET] qtaguid: %s(): erased %p\n", __func__, tr_entry);
		kfree(tr_entry);
	}
}
...
static void put_tag_ref_tree(tag_t full_tag, struct uid_tag_data *utd_entry)
{
	...
	DR_DEBUG("[NET] qtaguid: %s(tag=0x%llx (uid=%u))\n", __func__,
...
static int get_active_counter_set(tag_t tag)
{
	...
	MT_DEBUG("[NET] qtaguid: get_active_counter_set(tag=0x%llx)"
		 " (uid=%u)\n",
...
static struct iface_stat *get_iface_entry(const char *ifname)
{
	...
	if (ifname == NULL) {
		pr_info("[NET] qtaguid: iface_stat: get() NULL device name\n");
		return NULL;
	}
				    ...
				    int *eof, void *data)
{
	...
	CT_DEBUG("[NET] qtaguid:proc iface_stat_all "
...
static void iface_create_proc_worker(struct work_struct *work)
{
	...
	if (IS_ERR_OR_NULL(proc_entry)) {
		pr_err("[NET] qtaguid: iface_stat: create_proc(): alloc failed.\n");
		kfree(isw);
		return;
	}
	...
	IF_DEBUG("[NET] qtaguid: iface_stat: create_proc(): done "
...
static struct iface_stat *iface_alloc(struct net_device *net_dev)
{
	...
	if (new_iface == NULL) {
		pr_err("[NET] qtaguid: iface_stat: create(%s): "
					       ...
					       struct iface_stat *iface)
{
	...
	IF_DEBUG("[NET] qtaguid: %s(%s): iface=%p netdev=%p "
		 "bytes rx/tx=%llu/%llu "
	...
	if (iface->active && iface->last_known_valid && stats_rewound) {
		pr_warn_once("[NET] qtaguid: iface_stat: %s(%s): "
		...
		IF_DEBUG("[NET] qtaguid: %s(%s): iface=%p "
			      ...
			      struct in_ifaddr *ifa)
{
	...
	IF_DEBUG("[NET] qtaguid: iface_stat: create(%s): ifa=%p netdev=%p\n",
		 net_dev ? net_dev->name : "?",
		 ifa, net_dev);
	if (!net_dev) {
		pr_err("[NET] qtaguid: iface_stat: create(): no net dev\n");
		return;
	}
	...
	if (!ifa) {
		...
		if (!in_dev) {
			pr_err("[NET] qtaguid: iface_stat: create(%s): no inet dev\n",
			       ifname);
			return;
		}
		IF_DEBUG("[NET] qtaguid: iface_stat: create(%s): in_dev=%p\n",
			 ifname, in_dev);
		for (ifa = in_dev->ifa_list; ifa; ifa = ifa->ifa_next) {
			IF_DEBUG("[NET] qtaguid: iface_stat: create(%s): "
				 "ifa=%p ifa_label=%s\n",
				 ifname, ifa,
	...
	if (!ifa) {
		IF_DEBUG("[NET] qtaguid: iface_stat: create(%s): no matching IP\n",
			 ifname);
		goto done_put;
	}
	...
	if (entry != NULL) {
		...
		IF_DEBUG("[NET] qtaguid: iface_stat: create(%s): entry=%p\n",
			 ifname, entry);
	...
	IF_DEBUG("[NET] qtaguid: iface_stat: create(%s): done "
				   ...
				   struct inet6_ifaddr *ifa)
{
	...
	IF_DEBUG("[NET] qtaguid: iface_stat: create6(): ifa=%p netdev=%p->name=%s\n",
		 ifa, net_dev, net_dev ? net_dev->name : "");
	if (!net_dev) {
		pr_err("[NET] qtaguid: iface_stat: create6(): no net dev!\n");
		return;
	}
	ifname = net_dev->name;
	...
	in_dev = in_dev_get(net_dev);
	if (!in_dev) {
		pr_err("[NET] qtaguid: iface_stat: create6(%s): no inet dev\n",
		       ifname);
		return;
	}

	IF_DEBUG("[NET] qtaguid: iface_stat: create6(%s): in_dev=%p\n",
		 ifname, in_dev);

	if (!ifa) {
		IF_DEBUG("[NET] qtaguid: iface_stat: create6(%s): no matching IP\n",
			 ifname);
		goto done_put;
	}
	...
	if (entry != NULL) {
		...
		IF_DEBUG("[NET] qtaguid: %s(%s): entry=%p\n", __func__,
			 ifname, entry);
		iface_check_stats_reset_and_adjust(net_dev, entry);
		_iface_stat_set_active(entry, net_dev, activate);
		IF_DEBUG("[NET] qtaguid: %s(%s): "
	...
	new_iface = iface_alloc(net_dev);
	IF_DEBUG("[NET] qtaguid: iface_stat: create6(%s): done "
...
{
	MT_DEBUG("[NET] qtaguid: get_sock_stat_nl(sk=%p)\n", sk);
...
static struct sock_tag *get_sock_stat(const struct sock *sk)
{
	...
	MT_DEBUG("[NET] qtaguid: get_sock_stat(sk=%p)\n", sk);
	if (!sk)
		return NULL;
...
static void iface_stat_update(struct net_device *net_dev, bool stash_only)
{
	...
	if (entry == NULL) {
		IF_DEBUG("[NET] qtaguid: iface_stat: update(%s): not tracked\n",
			 net_dev->name);
		spin_unlock_bh(&iface_stat_list_lock);
		return;
	}
	...
	IF_DEBUG("[NET] qtaguid: %s(%s): entry=%p\n", __func__,
		 net_dev->name, entry);
	if (!entry->active) {
		IF_DEBUG("[NET] qtaguid: %s(%s): already disabled\n", __func__,
			 net_dev->name);
		spin_unlock_bh(&iface_stat_list_lock);
		return;
	}
			...
			enum ifs_tx_rx direction, int proto, int bytes)
{
	...
	MT_DEBUG("[NET] qtaguid: tag_stat_update(tag=0x%llx (uid=%u) set=%d "
					   ...
					   tag_t tag)
{
	...
	IF_DEBUG("[NET] qtaguid: iface_stat: %s(): ife=%p tag=0x%llx"
		 " (uid=%u)\n", __func__,
		 iface_entry, tag, get_uid_from_tag(tag));
	new_tag_stat_entry = kzalloc(sizeof(*new_tag_stat_entry), GFP_ATOMIC);
	if (!new_tag_stat_entry) {
		pr_err("[NET] qtaguid: iface_stat: tag stat alloc failed\n");
		goto done;
	}
			       ...
			       int proto, int bytes)
{
	...
	MT_DEBUG("[NET] qtaguid: iface_stat: stat_update() dev=%s entry=%p\n",
		 ifname, iface_entry);
	...
	MT_DEBUG("[NET] qtaguid: iface_stat: stat_update(): "
...
static int iface_netdev_event_handler(struct notifier_block *nb,
	...
	IF_DEBUG("[NET] qtaguid: iface_stat: netdev_event(): "
					 ...
					 unsigned long event, void *ptr)
{
	...
	IF_DEBUG("[NET] qtaguid: iface_stat: inet6addr_event(): "
		 "ev=0x%lx/%s ifa=%p\n",
					...
					unsigned long event, void *ptr)
{
	...
	IF_DEBUG("[NET] qtaguid: iface_stat: inetaddr_event(): "
		 "ev=0x%lx/%s ifa=%p\n",
		 event, netdev_evt_str(event), ifa);
...
static int __init iface_stat_init(struct proc_dir_entry *parent_procdir)
{
	...
	if (!iface_stat_procdir) {
		pr_err("[NET] qtaguid: iface_stat: init failed to create proc entry\n");
		err = -1;
		goto err;
	}
	...
	if (!iface_stat_all_procfile) {
		pr_err("[NET] qtaguid: iface_stat: init "
				    ...
				    struct xt_action_param *par)
{
	...
	MT_DEBUG("[NET] qtaguid: find_sk(skb=%p) hooknum=%d family=%d\n", skb,
	 ...
	 */
	if (sk) {
		MT_DEBUG("[NET] qtaguid: %p->sk_proto=%u "
				  ...
				  void *data)
{
	...
	CT_DEBUG("[NET] qtaguid: proc ctrl page=%p off=%ld char_count=%d *eof=%d\n",
	...
	for (node = rb_first(&sock_tag_tree);
		...
		CT_DEBUG("[NET] qtaguid: proc_read(): sk=%p tag=0x%llx (uid=%u) "
			 "pid=%u\n",
			 sock_tag_entry->sk,
			 sock_tag_entry->tag,
			 uid,
...
static int ctrl_cmd_delete(const char *input)
{
	...
	CT_DEBUG("[NET] qtaguid: ctrl_delete(%s): argc=%d cmd=%c "
	...
	if (!valid_atag(acct_tag)) {
		pr_info("[NET] qtaguid: ctrl_delete(%s): invalid tag\n", input);
		res = -EINVAL;
		goto err;
	}
	if (argc < 3) {
		...
		pr_info("[NET] qtaguid: ctrl_delete(%s): "
	...
	tag = combine_atag_with_uid(acct_tag, uid);
	CT_DEBUG("[NET] qtaguid: ctrl_delete(%s): "
	...
	while (node) {
		...
		CT_DEBUG("[NET] qtaguid: ctrl_delete(%s): st tag=0x%llx (uid=%u)\n",
...
static int ctrl_cmd_counter_set(const char *input)
{
	...
	CT_DEBUG("[NET] qtaguid: ctrl_counterset(%s): argc=%d cmd=%c "
		 "set=%d uid=%u\n", input, argc, cmd,
		 counter_set, uid);
	if (argc != 3) {
		res = -EINVAL;
	...
	if (counter_set < 0 || counter_set >= IFS_MAX_COUNTER_SETS) {
		pr_info("[NET] qtaguid: ctrl_counterset(%s): invalid counter_set range\n",
			input);
		res = -EINVAL;
		goto err;
	}
	if (!can_manipulate_uids()) {
		pr_info("[NET] qtaguid: ctrl_counterset(%s): "
			"insufficient priv from pid=%u tgid=%u uid=%u\n",
			input, current->pid, current->tgid, current_fsuid());
		res = -EPERM;
		goto err;
	...
	if (!tcs) {
		...
		if (!tcs) {
			spin_unlock_bh(&tag_counter_set_list_lock);
			pr_err("[NET] qtaguid: ctrl_counterset(%s): "
		...
		CT_DEBUG("[NET] qtaguid: ctrl_counterset(%s): added tcs tag=0x%llx "
			 "(uid=%u) set=%d\n",
...
static int ctrl_cmd_tag(const char *input)
{
	...
	CT_DEBUG("[NET] qtaguid: ctrl_tag(%s): argc=%d cmd=%c sock_fd=%d "
	...
	if (!el_socket) {
		pr_info("[NET] qtaguid: ctrl_tag(%s): failed to lookup"
			" sock_fd=%d err=%d\n", input, sock_fd, res);
		goto err;
	}
	CT_DEBUG("[NET] qtaguid: ctrl_tag(%s): socket->...->f_count=%ld ->sk=%p\n",
		 input, atomic_long_read(&el_socket->file->f_count),
		 el_socket->sk);
	if (argc < 3) {
		acct_tag = make_atag_from_value(0);
	} else if (!valid_atag(acct_tag)) {
		pr_info("[NET] qtaguid: ctrl_tag(%s): invalid tag\n", input);
		res = -EINVAL;
		goto err_put;
	}
	CT_DEBUG("[NET] qtaguid: ctrl_tag(%s): "
	...
	if (argc < 4) {
		...
		pr_info("[NET] qtaguid: ctrl_tag(%s): "
			"insufficient priv from pid=%u tgid=%u uid=%u\n",
			input, current->pid, current->tgid, current_fsuid());
		res = -EPERM;
		goto err_put;
	...
	if (sock_tag_entry) {
		...
		CT_DEBUG("[NET] qtaguid: ctrl_tag(%s): retag for sk=%p "
			 "st@%p ...->f_count=%ld\n",
		...
		CT_DEBUG("[NET] qtaguid: ctrl_tag(%s): newtag for sk=%p\n",
			 input, el_socket->sk);
		sock_tag_entry = kzalloc(sizeof(*sock_tag_entry),
					 GFP_ATOMIC);
		if (!sock_tag_entry) {
			pr_err("[NET] qtaguid: ctrl_tag(%s): "
	...
	CT_DEBUG("[NET] qtaguid: ctrl_tag(%s): done st@%p ...->f_count=%ld\n",
...
err_put:
	CT_DEBUG("[NET] qtaguid: ctrl_tag(%s): done. ...->f_count=%ld\n",
...
err:
	CT_DEBUG("[NET] qtaguid: ctrl_tag(%s): done.\n", input);
	return res;
}
...
static int ctrl_cmd_untag(const char *input)
{
	...
	CT_DEBUG("[NET] qtaguid: ctrl_untag(%s): argc=%d cmd=%c sock_fd=%d\n",
		 input, argc, cmd, sock_fd);
	if (argc < 2) {
		res = -EINVAL;
		goto err;
	...
	el_socket = sockfd_lookup(sock_fd, &res);  /* This locks the file */
	if (!el_socket) {
		pr_info("[NET] qtaguid: ctrl_untag(%s): failed to lookup"
			" sock_fd=%d err=%d\n", input, sock_fd, res);
		goto err;
	}
	CT_DEBUG("[NET] qtaguid: ctrl_untag(%s): socket->...->f_count=%ld ->sk=%p\n",
		 input, atomic_long_read(&el_socket->file->f_count),
		 el_socket->sk);
	spin_lock_bh(&sock_tag_list_lock);
	sock_tag_entry = get_sock_stat_nl(el_socket->sk);
	 ...
	 */
	sockfd_put(sock_tag_entry->socket);
	CT_DEBUG("[NET] qtaguid: ctrl_untag(%s): done. st@%p ...->f_count=%ld\n",
		 input, sock_tag_entry,
...
err_put:
	CT_DEBUG("[NET] qtaguid: ctrl_untag(%s): done. socket->...->f_count=%ld\n",
		 input, atomic_long_read(&el_socket->file->f_count) - 1);
	/* Release the sock_fd that was grabbed by sockfd_lookup(). */
	sockfd_put(el_socket);
	return res;
...
err:
	CT_DEBUG("[NET] qtaguid: ctrl_untag(%s): done.\n", input);
	return res;
}
...
static int qtaguid_ctrl_parse(const char *input, int count)
{
	...
	if (!res)
		res = count;
err:
	CT_DEBUG("[NET] qtaguid: ctrl(%s): res=%d\n", input, res);
	return res;
}
...
static int pp_stats_line(struct proc_print_info *ppi, int cnt_set)
{
	...
	if (!ppi->item_index) {
		...
		if (!can_read_other_uid_stats(stat_uid)) {
			CT_DEBUG("[NET] qtaguid: stats line: "
				...
				void *data)
{
	...
	CT_DEBUG("[NET] qtaguid:proc stats page=%p *num_items_returned=%p off=%ld "
		"char_count=%d *eof=%d\n", page, *num_items_returned,
		items_to_skip, char_count, *eof);
...
static int qtudev_open(struct inode *inode, struct file *file)
{
	...
	DR_DEBUG("[NET] qtaguid: qtudev_open(): pid=%u tgid=%u uid=%u\n",
		 current->pid, current->tgid, current_fsuid());
	...
	pqd_entry = proc_qtu_data_tree_search(&proc_qtu_data_tree,
					      current->tgid);
	if (pqd_entry) {
		pr_err("[NET] qtaguid: qtudev_open(): %u/%u %u "
		       "%s already opened\n",
		       current->pid, current->tgid, current_fsuid(),
		       QTU_DEV_NAME);
		res = -EBUSY;
	...
	spin_unlock_bh(&uid_tag_data_tree_lock);
	DR_DEBUG("[NET] qtaguid: tracking data for uid=%u in pqd=%p\n",
...
static int qtudev_release(struct inode *inode, struct file *file)
{
	 ...
	 */
	DR_DEBUG("[NET] qtaguid: qtudev_release(): "
		 "pid=%u tgid=%u uid=%u "
...
static int __init qtaguid_proc_register(struct proc_dir_entry **res_procdir)
{
	...
	*res_procdir = proc_mkdir(module_procdirname, init_net.proc_net);
	if (!*res_procdir) {
		pr_err("[NET] qtaguid: failed to create proc/.../xt_qtaguid\n");
		ret = -ENOMEM;
		goto no_dir;
	}
	...
	if (!xt_qtaguid_ctrl_file) {
		pr_err("[NET] qtaguid: failed to create xt_qtaguid/ctrl "
			" file\n");
		ret = -ENOMEM;
		goto no_ctrl_entry;
	}
	...
	if (!xt_qtaguid_stats_file) {
		pr_err("[NET] qtaguid: failed to create xt_qtaguid/stats "
			"file\n");
		ret = -ENOMEM;
		goto no_stats_entry;
	}
